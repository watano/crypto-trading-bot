...
export class PairStateExecution {
    ...
    * @param ticker
    */
   constructor(exchangeManager: any, orderCalculator: any, orderExecutor: any, logger: any, ticker: any = undefined) {
    ...
    */
   async onCancelPair(pairState: PairState): Promise<void> {
    ...
    * @returns {Promise<void>}
    */
   async onSellBuyPair(pairState: PairState): Promise<void> {
      ...
      if (!exchangeOrderStored) {
         this.logger.info(`Pair State: Create position open order: ${JSON.stringify([pairState.exchange, pairState.symbol, pairState.getState(), pairState.options])}`);
   ...
   async onClosePair(pairState: PairState): Promise<void> {
      const position = await this.exchangeManager.getPosition(pairState.exchange, pairState.symbol);
      ...
      const exchangeOrderStored = await this.managedPairStateOrder(pairState);
      if (!exchangeOrderStored) {
         this.logger.info(`Pair State: Create position close order: ${JSON.stringify([pairState.exchange, pairState.symbol])}`);
   ...
   async onPairStateExecutionTick(pairState: PairState): Promise<void> {
      ...
      if (moment().subtract(25, 'minutes').diff(pairState.getTime()) > 0) {
    ...
    */
   async pairStateExecuteOrder(pair: PairState): Promise<ExchangeOrder | undefined> {
      ...
      const myOrder = options && options.market === true ? Order.createMarketOrder(symbol, orderSize) : Order.createLimitPostOnlyOrderAutoAdjustedPriceOrder(symbol, orderSize);

      return this.orderExecutor.executeOrder(exchangeName, myOrder);
   }

   async executeCloseOrder(exchangeName: string, symbol: string, orderSize: number, options: any) {
      ...
      const order = options && options.market === true ? Order.createMarketOrder(symbol, exchangeOrderSize) : Order.createCloseOrderWithPriceAdjustment(symbol, exchangeOrderSize);

      return this.orderExecutor.executeOrder(exchangeName, order);
   }

   async extractManagedPairStateOrderFromOrders(pairState: PairState) {
   ...
   async managedPairStateOrder(pairState: PairState) {
      ...
      pairState.setExchangeOrder(undefined);
