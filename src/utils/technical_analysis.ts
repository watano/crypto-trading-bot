...
type IndicatorDescriptor = {
  key: string;
  indicator: string | ((source: any, indicator: any) => Promise<Record<string, any>> | Record<string, any>);
  source?: string;
...
export function calculateReadyIndicators(indicatorList: IndicatorDescriptor[], results: Record<string, any>) {
  ...
  return indicatorList
    .map(indicator => ({
      ...indicator,
      source: indicator.source || (sourceCandle.includes(indicator.indicator) ? 'candles' : 'close')
    }))
    .filter(({ key }) => !(key in results))
    .filter(({ source }) => source in results.candles[0] || source in results)
    .map(indicator => {
      const { indicator: indicatorName, source } = indicator;
      const sourceData =
        source && source in results.candles[0] ? results.candles.map((v: any) => v[source]) : results[source as string];
 ...
 */
export async function createIndicatorsLookback(
  lookbacks: Candle[],
  indicatorList: IndicatorDescriptor[]
 ...
 * https://www.fidelity.com/learning-center/trading-investing/technical-analysis/technical-indicator-guide/pivot-points-high-low
 */
export function getPivotPointsWithWicks(
