$(() => {
   ...
   $('.percent-input-group').on('click', 'button', function (e) {
      ...
      const percentageChange = Number.parseFloat($(this).val());

      const price = Number.parseFloat(input.val());
   ...
   $('.form-group-amount').on('keyup', 'input', function (e) {
      ...
      if (id === 'amount') {
         scope.find('#amount_currency').val((value * assetPrice).toFixed(getPrecision(Number.parseFloat(assetPrice))));
      } else {
