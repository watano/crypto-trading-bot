...
export class OrderCalculator {
   ...
   constructor(tickers: Tickers, logger: any, exchangeManager: ExchangeManager, pairConfig: PairConfig) {
   ...
   async calculateOrderSizeCapital(exchangeName: string, symbol: string, capital: OrderCapital): Promise<number | undefined> {
      ...
      let amountCurrency = balancePercent ? (exchange.getTradableBalance() * balancePercent) / 100 : capital.getCurrency();
   ...
   async calculateOrderSize(exchangeName: string, symbol: string): Promise<number | undefined> {
      ...
      return await this.calculateOrderSizeCapital(exchangeName, symbol, capital);
   }
