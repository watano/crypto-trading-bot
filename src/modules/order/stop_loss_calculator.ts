...
export class StopLossCalculator {
   ...
   async calculateForOpenPosition(exchange: string, position: { entry: number; side: string; symbol: string }, options: { percent: number } = { percent: 3 }): Promise<number | undefined> {
      const { tickers } = this;
