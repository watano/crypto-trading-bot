...
import Binance from '~/src/exchange/binance';
...
//FIXME
describe.skip('#binance exchange implementation', () => {
   it('limit order is created', () => {
      const orders: any = Binance.createOrderBody(Order.createLimitPostOnlyOrder('BTCUSD', 'long', 1337, 0.5));

      orders.newClientOrderId = undefined;
   ...
   it('limit order is created for negative values', () => {
      const orders: any = Binance.createOrderBody(Order.createLimitPostOnlyOrder('BTCUSD', 'short', 1337, -0.5));
   ...
   it('market order is created', () => {
      const orders: any = Binance.createOrderBody(Order.createMarketOrder('BTCUSD', -1337));
   ...
   it('stop order is created', () => {
      const orders: any = Binance.createOrderBody(Order.createStopLossOrder('BTCUSD', -1337, 12));
   ...
   const getEvent = (find: any) => JSON.parse(fs.readFileSync(`${__dirname}/binance/events.json`, 'utf8')).find(find);

   const getOrders = () => JSON.parse(fs.readFileSync(`${__dirname}/binance/orders.json`, 'utf8'));
});
